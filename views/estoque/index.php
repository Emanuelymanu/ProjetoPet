...
<<script>
    ...
    for (let i = 0; i < linhas.length; i++) { let colNome=linhas[i].getElementsByTagName("td")[1]; if (colNome) { let
        ...
        // Aqui você pode buscar os dados da movimentação via AJAX ou já ter os dados em JS
        // Exemplo: preencher o modal para edição
        // Supondo que você tenha um array movimentacoes com os dados
        var mov = null;
        ...
        // Para garantir que ao abrir para nova movimentação, o form volte ao modo de cadastro
        function abrirModalMovimentacao(tipo, id_produto, nome_produto) {
        document.getElementById('id_movimentacao').value = '';
        document.getElementById('actionMovimentacao').value = 'salvar_movimentacao';
        const modalTitle = document.getElementById('modalLabel');
