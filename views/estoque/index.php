...
// 1. Incluir o Controller (Ajuste o caminho conforme sua estrutura)
            ...
            <div class="card shadow-lg border-0">
                ...
                <div class="card-body p-4 p-md-5">
                    ...
                    <div class="d-flex justify-content-between mb-4">
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#modalCadastroMovimentacao" title="Registrar Nova Movimentação de Estoque">
                                    ...
                                    <?php foreach ($dadosProdutos as $produto): ?>
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <span class="badge bg-<?= getQuantidadeBadgeColor($produto->quantidade) ?> fs-6">
                                                    <?= htmlspecialchars($produto->quantidade) ?> un.
                                                </span>
                                            </td>
                                            <td class="text-center">
                                                <button type="button" class="btn btn-sm btn-success me-1" 
                                                        onclick="abrirModalMovimentacao('entrada', <?= $produto->id_produto ?>, '<?= htmlspecialchars($produto->nome) ?>')" title="Registrar Nova Entrada (Compra)">
                                                     <i class="bi bi-arrow-up-circle"></i> Entrada
                                                </button>
                                                
                                                <button type="button" class="btn btn-sm btn-danger me-1" 
                                                        onclick="abrirModalMovimentacao('saida', <?= $produto->id_produto ?>, '<?= htmlspecialchars($produto->nome) ?>')" title="Dar Baixa (Venda/Perda)">
                                                     <i class="bi bi-arrow-down-circle"></i> Saída
                                                </button>
                                                
                                                <button type="button" class="btn btn-sm btn-info text-white" 
                                                        onclick="editarProduto(<?= $produto->id_produto ?>)" title="Editar Nome, Preço ou Outros Detalhes">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header bg-primary text-white">
        <h5 class="modal-title" id="modalLabel"><i class="bi bi-arrow-left-right"></i> Registrar Movimentação</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="formMovimentacao" method="POST" action="../../routes/estoqueRoutes.php">
        <div class="modal-body">
            <input type="hidden" name="action" value="salvar_movimentacao">
                ...
                <select class="form-select" id="id_produto" name="id_produto" required>
                    ...
                    ?>
                        <option value="<?= htmlspecialchars($produto->id_produto) ?>">
                            <?= htmlspecialchars($produto->nome) ?> (Estoque: <?= $produto->quantidade ?>)
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="datetime-local" class="form-control" id="data_movimentacao" name="data_movimentacao" value="<?= date('Y-m-d\TH:i') ?>" required>
            </div>
...
<<script>
    ...
    const tabelaEstoque = document.querySelector("#tabelaEstoque tbody"); // A tabela principal
    ...
    if (searchInput && tabelaEstoque) {
        searchInput.addEventListener("keyup", function () {
            ...
            for (let i = 0; i < linhas.length; i++) {
                ...
                let colNome = linhas[i].getElementsByTagName("td")[1]; 
                
                if (colNome) {
                    let nome = colNome.textContent.toLowerCase();
    ...
    function editarProduto(id) {
        alert(`Redirecionando para a edição do Produto ID: ${id_movimentacao}`);
    }
</script>